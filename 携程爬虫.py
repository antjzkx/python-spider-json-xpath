...
def xiechengair(dcity,acity,date):
    date = date[0:4]+'-'+date[4:6]+'-'+date[6:8]

    headers = {
        'user - agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
        'referer':'https://flights.ctrip.com', #引用
    ...
    table = PrettyTable(["航空公司","飞机编号","出发时间","抵达时间","准点到达率","出发机场","侯机楼","抵达机场","出口",
                         "最低价"])
    lists = []
    for route in routeList:
        if len(route.get('legs'))==1:
            ...
            info["飞机编号"] = flight.get("flightNumber")
            info["出发时间"] = flight.get("departureDate")
            info["抵达时间"] = flight.get("arrivalDate")
            info["准点到达率"] = flight.get("punctualityRate")
            info["出发机场"] = flight.get('arrivalAirportInfo').get('airportName')
            info["侯机楼"]= flight.get('arrivalAirportInfo').get('terminal').get('name')
            info["抵达机场"] = flight.get('departureAirportInfo').get('airportName')
            info["出口"] = flight.get('departureAirportInfo').get('terminal').get('name')
